...
class PDF extends TCPDF
{
    ...
    function Header()
    {
        ...
        $this->SetXY($x += 40, $y);     $this->Cell(40, 5, 'Taxes to declare', 1, 0, '', false);
        $this->SetXY($x += 40, $y);     $this->Cell(40, 5, 'Financed taxes', 1, 0, '', false);
...
$datos = $objReportes->resumenDeImpuestosPorDeclarar($fechaInicial, $fechaFinal, $sucursalId);
...
{
    $filaConteo++;
    $totalPorDeclarar += $fila["SUMAIMPUESTOPORDECLARAR"];
    $totalNoDeclarar += $fila["SUMAIMPUESTOSINDECLARAR"];

    $arrayFila = [
        $filaConteo,
        $fila["FECHA"],
        "$ " . number_format($fila["SUMAIMPUESTOPORDECLARAR"], 2, ".", ","),
        "$ " . number_format($fila["SUMAIMPUESTOSINDECLARAR"], 2, ".", ",")
    ];
...
$pdf->setXY(62, $currentY - 20); $pdf->Cell(40, 5, "$ " . number_format($totalPorDeclarar, 2, ".", ","), 1, 0, "R");
$pdf->setXY(102, $currentY - 20); $pdf->Cell(40, 5, "$ " . number_format($totalNoDeclarar, 2, ".", ","), 1, 0, "R");
