...
const selectSucursalOrigen = document.getElementById('sucursalorigen');
const spanNombreSucursal = document.getElementById('nombresucursal');
selectSucursalOrigen.addEventListener('change', function() {
    const textoSeleccionado = selectSucursalOrigen.options[selectSucursalOrigen.selectedIndex].text;
...
{
    if (document.getElementById("sucursalorigen").value == "")
    {
        document.getElementById("textodeerror").innerHTML = "You must select origin store to add inventory items.";
        toastError.show();
        return;
    }
    ...
    celda6.innerHTML = `<input type="text" id="msrp[]" name="msrp[]" class="form-control form-control-sm form-control-readonly" value="" onfocus="readonlyPre(event)" oninput="readonly(event)" readonly required>`;
    celda7.innerHTML = `<input type="text" id="stocktype[]" name="stocktype[]" class="form-control form-control-sm form-control-readonly" value="" onfocus="readonlyPre(event)" oninput="readonly(event)" readonly required>`;
    ...
    // La sucursal de origen no puede ser cambiada si ya hay al menos un ítem de inventario
    document.getElementById('sucursalorigen').style.pointerEvents = 'none';
    document.getElementById('sucursalorigen').style.backgroundColor = '#e9ecef';
...
function eliminarFila(boton)
{
    ...
    if (conteoDeFilas == 0)
    {
        document.getElementById('sucursalorigen').style.pointerEvents = 'auto';
        document.getElementById('sucursalorigen').style.backgroundColor = '';
    }
...
function buscarItem(input)
{
    if(input.value.length == 9)
    {
        ...
        fetch(
            "./mods/inventario/traslados/procs/buscaritem.php",
            {
                method: "POST",
                body: datos
            }
        ...
        .then(data => {
            ...
            if (data.encontrado == 1)
            {
                if (validarItem(data))
                {
                    ...
                    filaDetalleActual.cells[5].getElementsByTagName("input")[0].value = data.msrp;
                    filaDetalleActual.cells[6].getElementsByTagName("input")[0].value = data.tipodestockdist;
...
{
    // Ver que el ítem se encuentre en la sucursal origen
    if (data.sucursalid != document.getElementById("sucursalorigen").value)
    {
        document.getElementById("textodeerror").innerHTML = "The item is not found at the origin store.";
        toastError.show();
        return false;
    }
...
window.inventarioOperateEvents = {
    "click .sel-inventario": function(e, value, row, index) {
        ...
        filaDetalleActual.parentNode.cells[5].getElementsByTagName("input")[0].value = row.MSRP;
        filaDetalleActual.parentNode.cells[6].getElementsByTagName("input")[0].value = row.TIPODESTOCKDIST;
...
document.getElementById("frm").addEventListener("submit", (event) => {
    ...
    if (document.getElementById("sucursalorigen").value == document.getElementById("sucursaldestino").value)
    {
        document.getElementById("textodeerror").innerHTML = "The destination can not be the same as the origin.";
        toastError.show();
        return;
    }
...
function guardar(datos)
{
    ...
    fetch(
        "./mods/inventario/traslados/procs/guardartraslado.php",
        {
            method: "POST",
            body: datos
        }
...
function finalizar(data)
{
    ...
    if (data.error == 0)
    {
        toastMensaje.show();
        setTimeout(() => {
            window.location.href="?mod=inventario&opc=traslados&subopc=vertraslado&tid=" + data.tid;
        }, 2000);
    }
    else
    {
