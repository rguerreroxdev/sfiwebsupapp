...
$accesoEliminar = $objAccesos->getAccesoAOpcion("02.01.06.03");

if ($accesoEliminar)
{
    $objTiposDeStock = new TiposDeStock($conn);

    $rsEliminacion = $objTiposDeStock->eliminarRegistro($tipoDeStockId);
    
    if (!$rsEliminacion)
    {
        $resultado["error"] = 1;
        $resultado["mensaje"] = $objTiposDeStock->mensajeError;
    }
}
else
{
