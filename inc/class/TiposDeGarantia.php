...
class TiposDeGarantias
{
    ...
    /**
     * Instancia un objeto TiposDeGarantia
     * 
    ...
    /**
     * Obtener todos los registros de la tabla (INVTIPOSDEGARANTIA) con paginación
     * 
    ...
    public function getAllConPaginacion(string $nombre, int $numeroDePagina = 1, int $tamanoDePagina = 25): array
    {
        ...
        if ($nombre != "")
        {
            $condicion .= " WHERE TG.NOMBRE LIKE '%$nombre%' ";
        }
        ...
        $sentenciaSql = "
            SELECT
                COUNT(TG.TIPODEGARANTIAID) AS CONTEO
            FROM
                INVTIPOSDEGARANTIA TG
    ...
    /**
     * Obtener datos de un registro (INVTIPOSDEGARANTIA) por medio de ID
     * 
    ...
    public function getById(int $id): void
    {
        $sentenciaSql = "
            SELECT
                ...
                TG.NOMBRE,
                TG.FECHACREACION,
                TG.FECHAMODIFICACION,
                TG.USUARIOIDCREACION,
                UC.USUARIO AS USUARIOCREO,
                TG.USUARIOIDMODIFICACION,
                UM.USUARIO AS USUARIOMODIFICA
            FROM
                INVTIPOSDEGARANTIA TG
                JOIN ACCUSUARIOS UC ON UC.USUARIOID=TG.USUARIOIDCREACION
                JOIN ACCUSUARIOS UM ON UM.USUARIOID=TG.USUARIOIDMODIFICACION
            WHERE
                TG.TIPODEGARANTIAID = ?
        ";
    ...
    /**
     * Agregar un nuevo registro (INVTIPOSDEGARANTIA)
     * 
    ...
    public function agregarRegistro(string $nombre, int $usuarioId): bool
    {
        ...
        if ($agregado)
        {
            $this->tipoDeGarantiaId = $datoResultado[0]["ID"];
        }
        else
        {
    ...
    /**
     * Edita un registro (INVTIPOSDEGARANTIA) existente
     * 
    ...
    public function editarRegistro(int $id, array $camposValores): bool
    {
        ...
        $sentenciaSql = "
            UPDATE INVTIPOSDEGARANTIA SET " . $updates . " WHERE TIPODEGARANTIAID = ?
        ";
    ...
    /**
     * Eliminar un registro (INVTIPOSDEGARANTIA)
     * 
    ...
    public function eliminarRegistro(int $id): bool
    {
        ...
        $sentenciaSql = "
            DELETE FROM INVTIPOSDEGARANTIA WHERE TIPODEGARANTIAID = ?
        ";
    ...
    /**
     * Obtener todos los registros de la tabla (INVTIPOSDEGARANTIA) para mostrar en combo
