...
class PDF extends TCPDF
{
    ...
    function Header()
    {
        ...
        $this->SetXY(15, 20);	$this->Cell(0, 5, "INVENTORY DISCHARGE", 0, 0, "C");

        // Fecha y hora de generación
        $fechaDeEmision = new DateTime();
        $this->SetFont("Helvetica", "", 6);
		$this->SetXY(240, 20);	$this->Cell(30, 5, $fechaDeEmision->format("m/d/Y H:i"), 0, 0, 'R');

        // Datos de Recepción
        $this->SetFont("Helvetica", "", 8);
        $this->SetXY(15, 25);	$this->Cell(150, 4, 'Store: ' . $objSalida->sucursal, 0, 0);
        $this->SetXY(15, 33);	$this->Cell(50, 4, 'Correlative: ' . $objSalida->correlativo, 0, 0);
        $this->SetXY(80, 33);	$this->Cell(50, 4, 'Status: ' . $objSalida->nombreDeEstado, 0, 0);
        $this->SetXY(15, 37);	$this->Cell(50, 4, 'Discharge date: ' . $objSalida->fecha, 0, 0);
        $this->SetXY(80, 37);	$this->Cell(50, 4, 'Type: ' . $objSalida->tipoDeSalida, 0, 0);
        $this->SetXY(15, 41);	$this->MultiCell(185, 4, 'Concept: ' . $objSalida->concepto, 0, 0);
        ...
        $this->SetXY($x +=  6, $y);     $this->Cell(20, 5, 'Item code', 1, 0, '', false);
        $this->SetXY($x += 20, $y);     $this->Cell(30, 5, 'Category', 1, 0, '', false);
        $this->SetXY($x += 30, $y);     $this->Cell(20, 5, 'Brand', 1, 0, '', false);
        $this->SetXY($x += 20, $y);     $this->Cell(30, 5, 'Model', 1, 0, '', false);
        $this->SetXY($x += 30, $y);     $this->Cell(70, 5, 'Description', 1, 0, '', false);
        $this->SetXY($x += 70, $y);     $this->Cell(15, 5, 'MSRP', 1, 0, '', false);
        $this->SetXY($x += 15, $y);     $this->Cell(15, 5, 'Stk. type orig.', 1, 0, '', false);
        $this->SetXY($x += 15, $y);     $this->Cell(15, 5, 'Cost orig.', 1, 0, '', false);
        $this->SetXY($x += 15, $y);     $this->Cell(15, 5, 'Stk. type distr.', 1, 0, '', false);
        $this->SetXY($x += 15, $y);     $this->Cell(15, 5, 'Cost distr.', 1, 0, '', false);
...
$datos = $objSalidaDetalle->getAll($salidaId);
...
$anchoDeColumnas = [6, 20, 30, 20, 30, 70, 15, 15, 15, 15, 15];
$alineacionDeCelda = ["L", "L", "L", "L", "L", "L", "R", "L", "R", "L", "R"];
...
$pdf->Output("Inventory discharge " . $objSalida->correlativo . ".pdf");
