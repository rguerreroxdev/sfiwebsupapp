<?php
    ...
    {
        // Se está editando
        $accesoAOpcion = $usuarioAccesos->validarAccesoAOpcionDeMenu("INV", "02.02.03.02");
    }
    else
    {
        // Se está creando
        $accesoAOpcion = $usuarioAccesos->validarAccesoAOpcionDeMenu("INV", "02.02.03.01");
    }
    ...
    {   // Se cierra al final del archivo
        ...
        if ($objSalida->salidaId != -1 && $objSalida->estado != "FOR")
        {
            echo('<script>window.location.href="?mod=inventario&opc=salidas&subopc=versalida&sid=' . $objSalida->salidaId . '";</script>');
            exit();
        }
        ...
        {
            $objSalida->iniciarDatosParaNuevoRegistro();
        }
        else
        {
            require_once("mods/inventario/salidas/procs/getfilasdetalle.php");
            $filasDetalle = getFilasDetalle($conn, $objSalida->salidaId);
        }
        ...
        foreach ($listaDeSucursales as $sucursal)
        {
            ...
            $selected = "";
            if ($objSalida->sucursalId != -1)
            {
                $selected = $objSalida->sucursalId == $valor ? " selected" : "";
            }
        ...
        // Para crear el combo de tipos de salida
        require_once("inc/class/TiposDeSalida.php");
        $objTiposDeSalida = new TiposDeSalidas($conn);
        $listaDeTiposDeSalida = $objTiposDeSalida->getListaParaCombo("SELECT");
        $listaDeTiposDeSalidaOptions = "";
        foreach ($listaDeTiposDeSalida as $tipoDeSalida)
        {
            $texto = $tipoDeSalida["NOMBRE"];
            $valor = $tipoDeSalida["TIPODESALIDAID"] == -1 ? "" : $tipoDeSalida["TIPODESALIDAID"];

            $selected = "";
            if ($objSalida->tipoDeSalida != -1)
            {
                $selected = $objSalida->tipoDeSalidaId == $valor ? " selected" : "";
            }
...
<h3>Inventory discharges - Document registration</h3>
    ...
    <div class="tab-pane fade show active" id="nav-editar" role="tabpanel" aria-labelledby="nav-editar-tab" tabindex="0">
        <div class="p-3 bg-body rounded shadow-sm">
            ...
            <div class="row mt-2">
                ...
                <div class="col-md-5 col-lg-3">
                    <div class="input-group input-group-sm">
                        <span class="input-group-text width-130px">Discharge date<span class="text-danger">&nbsp;*</span></span>
                        <input type="date" id="fechadesalida" name="fechadesalida" class="form-control form-control-sm" value="<?= $objSalida->fechadt->format("Y-m-d") ?>" required>
                    </div>
                </div>
            </div>
            ...
            <div class="row mt-2">
                <div class="col-md-11 col-lg-8">
                    <div class="input-group input-group-sm">
                        ...
                        <textarea id="concepto" name="concepto" class="form-control form-control-sm" maxlength="250"><?= $objSalida->concepto ?></textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-md-5 col-lg-4">
                    <div class="input-group input-group-sm">
                        ...
                        <input type="text" id="fechacreacion" name="fechacreacion" class="form-control form-control-sm" value="<?= $objSalida->fechaCreacion == null ? date("m/d/Y") : $objSalida->fechaCreacion->format("m/d/Y") ?>" readonly>
                    </div>
                </div>
                <div class="col-md-5 col-lg-3">
                    <div class="input-group input-group-sm">
                        <span class="input-group-text width-160px">User who created</span>
                        <input type="text" id="usuariocreo" name="usuariocreo" class="form-control form-control-sm" value="<?= $objSalida->usuarioCreo == null ? $_SESSION["usuario"] : $objSalida->usuarioCreo ?>" readonly>
                    </div>
                </div>
            </div>
            <div class="row mt-2">
                <div class="col-md-5 col-lg-4">
                    <div class="input-group input-group-sm">
                        ...
                        <input type="text" id="fechamodificacion" name="fechamodificacion" class="form-control form-control-sm" value="<?= $objSalida->fechaModificacion == null ? date("m/d/Y") : $objSalida->fechaModificacion->format("m/d/Y") ?>" readonly>
                    </div>
                </div>
                <div class="col-md-5 col-lg-3">
                    <div class="input-group input-group-sm">
                        <span class="input-group-text width-160px">User who modified</span>
                        <input type="text" id="usuariomodifico" name="usuariomodifico" class="form-control form-control-sm" value="<?= $objSalida->usuarioModifica == null ? $_SESSION["usuario"] : $objSalida->usuarioModifica ?>" readonly>
                    </div>
                </div>
            </div>
        </div>
            ...
            >
                <thead>
                    <tr>
                        ...
                        <th data-field="SALIDAID" data-visible="false">TID</th>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <table
                        id="tableinventario"
                        data-toggle="table"
                        data-url="./mods/inventario/salidas/procs/getinventario.php"
