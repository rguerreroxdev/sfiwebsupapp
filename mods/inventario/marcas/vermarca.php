<?php
    ...
    {   // Se cierra al final del archivo
        ...
        // Obtener los datos de la marca recibida en la URL
        $marcaId = -1;
        if (isset($_GET["mid"]))
        {
            $marcaId = is_numeric($_GET["mid"]) ? intval($_GET["mid"]) : -1;
        }
        
        require_once("inc/class/Marcas.php");
        $objMarcas = new Marcas($conn);

        $objMarcas->getById($marcaId);

        // Si el registro no existe, retornar a listado
        if ($objMarcas->marcaId == -1)
        {
            echo ("<script>window.location.href='?mod=inventario&opc=marcas'</script>");
            exit();
        }

        // Obtener accesos a opciones sobre registro
        $accesos = $usuarioAccesos->getListaDeOpcionesConAcceso("02.01.04");
        $accesoEditar = in_array("02.01.04.02", $accesos) ? "" : " disabled";
        $accesoEliminar = in_array("02.01.04.03", $accesos) ? "" : " disabled";
?>

<h3>Brands - Brand data</h3>

<nav>
    <div class="nav nav-tabs small" id="nav-tab" role="tablist">
        <button class="nav-link active" id="nav-editar-tab" data-bs-toggle="tab" data-bs-target="#nav-editar" type="button" role="tab" aria-controls="nav-editar" aria-selected="true">Brand data</button>
            ...
            <div class="row">
                <div class="col-md-5 col-lg-3">
                    <div class="input-group input-group-sm">
                        ...
                        <input type="text" id="nombre" name="nombre" class="form-control form-control-sm" value="<?= $objMarcas->nombre ?>" readonly>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="p-3 bg-body rounded shadow-sm">
            <div class="row">
                <div class="col-md-5 col-lg-4">
                    <div class="input-group input-group-sm">
                        <span class="input-group-text width-160px">Date of creation</span>
                        <input type="text" id="fechacreacion" name="fechacreacion" class="form-control form-control-sm" value="<?= $objMarcas->fechaCreacion->format("m/d/Y H:i") ?>" readonly>
                    </div>
                </div>
                <div class="col-md-5 col-lg-3">
                    <div class="input-group input-group-sm">
                        <span class="input-group-text width-160px">User who created</span>
                        <input type="text" id="usuariocreo" name="usuariocreo" class="form-control form-control-sm" value="<?= $objMarcas->usuarioCreo ?>" readonly>
                    </div>
                </div>
            </div>
            <div class="row mt-2">
                <div class="col-md-5 col-lg-4">
                    <div class="input-group input-group-sm">
                        ...
                        <input type="text" id="fechamodificacion" name="fechamodificacion" class="form-control form-control-sm" value="<?= $objMarcas->fechaModificacion->format("m/d/Y H:i") ?>" readonly>
                    </div>
                </div>
                <div class="col-md-5 col-lg-3">
                    <div class="input-group input-group-sm">
                        <span class="input-group-text width-160px">User who modified</span>
                        <input type="text" id="usuariomodifico" name="usuariomodifico" class="form-control form-control-sm" value="<?= $objMarcas->usuarioModifica ?>" readonly>
                    </div>
                </div>
            </div>
        </div>

        <input type="hidden" id="mid" name="mid" value="<?= $objMarcas->marcaId ?>">
