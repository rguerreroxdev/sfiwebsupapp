<?php
    ...
    $accesoAOpcion = $usuarioAccesos->validarAccesoAOpcionDeMenu("INV", "02.01.05");
    ...
    {   // Se cierra al final del archivo
        ...
        // Obtener los datos del color recibido en la URL
        $colorId = -1;
        if (isset($_GET["cid"]))
        {
            $colorId = is_numeric($_GET["cid"]) ? intval($_GET["cid"]) : -1;
        }
        ...
        $objColores->getById($colorId);

        // Si el registro no existe, retornar a listado
        if ($objColores->colorId == -1)
        {
            echo ("<script>window.location.href='?mod=inventario&opc=colores'</script>");
            exit();
        }

        // Obtener accesos a opciones sobre registro
        $accesos = $usuarioAccesos->getListaDeOpcionesConAcceso("02.01.05");
        $accesoEditar = in_array("02.01.05.02", $accesos) ? "" : " disabled";
        $accesoEliminar = in_array("02.01.05.03", $accesos) ? "" : " disabled";
?>

<h3>Colors - Color data</h3>

<nav>
    <div class="nav nav-tabs small" id="nav-tab" role="tablist">
        <button class="nav-link active" id="nav-editar-tab" data-bs-toggle="tab" data-bs-target="#nav-editar" type="button" role="tab" aria-controls="nav-editar" aria-selected="true">Color data</button>
            ...
            <div class="row">
                <div class="col-md-5 col-lg-3">
                    <div class="input-group input-group-sm">
                        ...
                        <input type="text" id="nombre" name="nombre" class="form-control form-control-sm" value="<?= $objColores->nombre ?>" readonly>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                <div class="col-md-5 col-lg-4">
                    <div class="input-group input-group-sm">
                        ...
                        <input type="text" id="fechacreacion" name="fechacreacion" class="form-control form-control-sm" value="<?= $objColores->fechaCreacion->format("m/d/Y H:i") ?>" readonly>
                    </div>
                </div>
                <div class="col-md-5 col-lg-3">
                    <div class="input-group input-group-sm">
                        <span class="input-group-text width-160px">User who created</span>
                        <input type="text" id="usuariocreo" name="usuariocreo" class="form-control form-control-sm" value="<?= $objColores->usuarioCreo ?>" readonly>
                    </div>
                </div>
            </div>
            <div class="row mt-2">
                <div class="col-md-5 col-lg-4">
                    <div class="input-group input-group-sm">
                        ...
                        <input type="text" id="fechamodificacion" name="fechamodificacion" class="form-control form-control-sm" value="<?= $objColores->fechaModificacion->format("m/d/Y H:i") ?>" readonly>
                    </div>
                </div>
                <div class="col-md-5 col-lg-3">
                    <div class="input-group input-group-sm">
                        <span class="input-group-text width-160px">User who modified</span>
                        <input type="text" id="usuariomodifico" name="usuariomodifico" class="form-control form-control-sm" value="<?= $objColores->usuarioModifica ?>" readonly>
                    </div>
                </div>
            </div>
        </div>

        <input type="hidden" id="cid" name="cid" value="<?= $objColores->colorId ?>">
