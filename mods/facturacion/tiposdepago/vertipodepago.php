<?php
    ...
    $accesoAOpcion = $usuarioAccesos->validarAccesoAOpcionDeMenu("ADM", "03.01.04");
    ...
    {   // Se cierra al final del archivo
        ...
        // Obtener los datos del tipo de pago recibido en la URL
        $tipoDePagoId = -1;
        if (isset($_GET["tpid"]))
        {
            $tipoDePagoId = is_numeric($_GET["tpid"]) ? intval($_GET["tpid"]) : -1;
        }


        require_once("inc/class/TiposDePago.php");
        $objTipoDePago = new TiposDePago($conn);

        $objTipoDePago->getById($tipoDePagoId);

        // Si el registro no existe, retornar a listado
        if ($objTipoDePago->tipoDePagoId == -1)
        {
            echo ("<script>window.location.href='?mod=facturacion&opc=tiposdepago'</script>");
            exit();
        }

        // Obtener accesos a opciones sobre registro
        $accesos = $usuarioAccesos->getListaDeOpcionesConAcceso("03.01.04");
        $accesoEditar = in_array("03.01.04.02", $accesos) ? "" : " disabled";
        $accesoEliminar = in_array("03.01.04.03", $accesos) ? "" : " disabled";
?>

<h3>Forms of payment - Form of payment data</h3>
    ...
    <div class="tab-pane fade show active" id="nav-editar" role="tabpanel" aria-labelledby="nav-editar-tab" tabindex="0">
        <div class="p-3 bg-body rounded shadow-sm">
            <div class="row">
                <div class="col-sm-5 col-lg-4">
                    <div class="input-group input-group-sm">
                        ...
                        <input type="text" id="nombre" name="nombre" class="form-control form-control-sm" value="<?= $objTipoDePago->nombre ?>" readonly>
                    </div>
                </div>
            </div>
            ...
            <div class="row mt-2">
                <div class="col-sm-5 col-lg-4">
                    <div class="input-group input-group-sm">
                        <span class="input-group-text width-130px">Add tax to reports</span>
                        <div class="input-group-text">
                            <input type="checkbox" id="sumaimpuesto" name="sumaimpuesto" class="form-check-input mt-0" value=""<?= $objTipoDePago->sumaImpuesto == 1 ? " checked" : "" ?> disabled>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="row mt-2">
                <div class="col-sm-5 col-lg-4">
                    <div class="input-group input-group-sm">
                        <span class="input-group-text width-200px">Can add payments without tax</span>
                        <div class="input-group-text">
                            <input type="checkbox" id="pagosinimpuesto" name="pagosinimpuesto" class="form-check-input mt-0" value=""<?= $objTipoDePago->permitePagoSinImpuesto == 1 ? " checked" : "" ?> disabled>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-sm-4">
                    <div class="input-group input-group-sm">
                        ...
                        <input type="text" id="fechacreacion" name="fechacreacion" class="form-control form-control-sm" value="<?= $objTipoDePago->fechaCreacion->format("m/d/Y H:i") ?>" readonly>
                    </div>
                </div>
                <div class="col-sm-3">
                    <div class="input-group input-group-sm">
                        <span class="input-group-text width-160px">User who created</span>
                        <input type="text" id="usuariocreo" name="usuariocreo" class="form-control form-control-sm" value="<?= $objTipoDePago->usuarioCreo ?>" readonly>
                    </div>
                </div>
            </div>
            <div class="row mt-2">
                <div class="col-sm-4">
                    <div class="input-group input-group-sm">
                        ...
                        <input type="text" id="fechamodificacion" name="fechamodificacion" class="form-control form-control-sm" value="<?= $objTipoDePago->fechaModificacion->format("m/d/Y H:i") ?>" readonly>
                    </div>
                </div>
                <div class="col-sm-3">
                    <div class="input-group input-group-sm">
                        <span class="input-group-text width-160px">User who modified</span>
                        <input type="text" id="usuariomodifico" name="usuariomodifico" class="form-control form-control-sm" value="<?= $objTipoDePago->usuarioModifica ?>" readonly>
                    </div>
                </div>
            </div>
        </div>

        <input type="hidden" id="tpid" name="tpid" value="<?= $objTipoDePago->tipoDePagoId ?>">
