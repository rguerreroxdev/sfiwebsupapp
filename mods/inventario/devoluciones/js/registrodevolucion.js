...
document.getElementById("btncancelar").addEventListener("click", () => {
    ...
    if (dId == -1)
    {
        window.location.href="?mod=inventario&opc=devoluciones";
    }
    else
    {
        window.location.href="?mod=inventario&opc=devoluciones&subopc=verdevolucion&did=" + dId;
    }
});
...
function agregarFila()
{
    ...
    celda6.innerHTML = `<input type="text" id="salidanumero[]" name="salidanumero[]" class="form-control form-control-sm form-control-readonly" value="" onfocus="readonlyPre(event)" oninput="readonly(event)" readonly required>`;
    celda7.innerHTML = `<input type="text" id="salidafecha[]" name="salidafecha[]" class="form-control form-control-sm form-control-readonly" value="" onfocus="readonlyPre(event)" oninput="readonly(event)" readonly required>`;
    celda8.innerHTML = `<input type="text" id="tipodesalida[]" name="tipodesalida[]" class="form-control form-control-sm form-control-readonly" value="" onfocus="readonlyPre(event)" oninput="readonly(event)" readonly required>`;
    celda9.innerHTML = `<input type="hidden" id="detalleid[]" name="detalleid[]" value=""><input type="hidden" id="salidadetalleid[]" name="salidadetalleid[]" value="">
...
function eliminarFila(boton)
{
    ...
    detid = fila.cells[8].getElementsByTagName("input")[0].value;
    if (detid != "")
    {
...
function buscarItem(input)
{
    if(input.value.length == 9)
    {
        ...
        fetch(
            "./mods/inventario/devoluciones/procs/buscaritem.php",
            {
                method: "POST",
                body: datos
            }
        ...
        .then(data => {
            ...
            if (data.encontrado == 1)
            {
                if (validarItem(data))
                {
                    ...
                    filaDetalleActual.cells[5].getElementsByTagName("input")[0].value = data.correlativosalida;
                    filaDetalleActual.cells[6].getElementsByTagName("input")[0].value = data.fechasalida;
                    filaDetalleActual.cells[7].getElementsByTagName("input")[0].value = data.tipodesalida;
                    filaDetalleActual.cells[8].getElementsByTagName("input")[1].value = data.salidadetalleid;
                ...
                else
                {
                    ...
                    filaDetalleActual.cells[6].getElementsByTagName("input")[0].value = "";
                    filaDetalleActual.cells[7].getElementsByTagName("input")[0].value = "";
                    filaDetalleActual.cells[8].getElementsByTagName("input")[1].value = "";
            ...
            {
                document.getElementById("textodeerror").innerHTML = "The item is not found as a discharge at the selected store.";
                toastError.show();
            }
...
window.inventarioOperateEvents = {
    "click .sel-inventario": function(e, value, row, index) {
        ...
        filaDetalleActual.parentNode.cells[5].getElementsByTagName("input")[0].value = row.CORRELATIVOSALIDA;
        filaDetalleActual.parentNode.cells[6].getElementsByTagName("input")[0].value = row.FECHASALIDA;
        filaDetalleActual.parentNode.cells[7].getElementsByTagName("input")[0].value = row.TIPODESALIDA;
        filaDetalleActual.parentNode.cells[8].getElementsByTagName("input")[1].value = row.SALIDADETALLEID;
...
function guardar(datos)
{
    ...
    fetch(
        "./mods/inventario/devoluciones/procs/guardardevolucion.php",
        {
            method: "POST",
            body: datos
        }
...
function finalizar(data)
{
    ...
    if (data.error == 0)
    {
        toastMensaje.show();
        setTimeout(() => {
            window.location.href="?mod=inventario&opc=devoluciones&subopc=verdevolucion&did=" + data.did;
        }, 2000);
    }
    else
    {
