...
class PDF extends TCPDF
{
    ...
    function Header()
    {
        ...
        $this->SetXY(15, 20);	$this->Cell(0, 5, "INVENTORY TRANSLATED TO STORES", 0, 0, "C");
...
foreach ($datos as $fila)
{
    ...
    $arrayFila = [
        ...
        "$ " . number_format($fila["TOTALCOSTOORIGEN"], 2),
        $fila["TIPODESTOCKDISTR"],
        "$ " . number_format($fila["TOTALCOSTODISTR"], 2)
    ];

    array_push($arrayDatosLimpios, $arrayFila);

    $cantidadTotal += $fila["CANTIDAD"];
    $granTotalCostoOrigen += $fila["TOTALCOSTOORIGEN"];
    $granTotalCostoDistr += $fila["TOTALCOSTODISTR"];
}
