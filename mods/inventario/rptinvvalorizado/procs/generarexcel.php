...
array_push($data, ["<b>Valued inventory</b>"]);
...
array_push($data, [
    ...
    "<b>MSRP $</b>", "<b>Stock type orig.</b>", "<b>Stock</b>", "<b>In transit</b>", "<b>Cost</b>"
]);
...
{
    $conteo++;
    $totalCosto += $dato["COSTOORIGEN"];

    array_push($data, [
        $conteo,
        $dato["SUCURSAL"], $dato["CODIGOINVENTARIO"], $dato["CATEGORIA"], $dato["MARCA"],
        $dato["MODELO"], $dato["COLOR"], $dato["DESCRIPCION"],
        $dato["MSRP"], $dato["TIPODESTOCKORIGEN"], $dato["EXISTENCIA"], $dato["ENTRANSITO"], $dato["COSTOORIGEN"]
    ]);
}
...
array_push($data, [
    "Total items:", $conteo, "", "", "",
    "", "", "",
    "", "", "", "Total cost:", number_format($totalCosto, 2)
]);
...
$xlsx->downloadAs('Valued inventory.xlsx');