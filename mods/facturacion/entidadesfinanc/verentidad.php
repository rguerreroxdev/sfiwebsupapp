<?php
    ...
    {   // Se cierra al final del archivo
        ...
        // Obtener los datos de la entidad financiera recibida en la URL
        $entidadFinancieraId = -1;
        if (isset($_GET["efid"]))
        {
            $entidadFinancieraId = is_numeric($_GET["efid"]) ? intval($_GET["efid"]) : -1;
        }
        ...
        $objEntidadFinanciera->getById($entidadFinancieraId);

        // Si el registro no existe, retornar a listado
        if ($objEntidadFinanciera->financieraId == -1)
        {
            echo ("<script>window.location.href='?mod=facturacion&opc=entidadesfinanc'</script>");
            exit();
        }

        // Obtener accesos a opciones sobre registro
        $accesos = $usuarioAccesos->getListaDeOpcionesConAcceso("03.01.03");
        $accesoEditar = in_array("03.01.03.02", $accesos) ? "" : " disabled";
        $accesoEliminar = in_array("03.01.03.03", $accesos) ? "" : " disabled";
?>

<h3>Financial entities - Entity data</h3>
            ...
            <div class="row">
                <div class="col-md-6 col-lg-4">
                    <div class="input-group input-group-sm">
                        ...
                        <input type="text" id="nombre" name="nombre" class="form-control form-control-sm" value="<?= $objEntidadFinanciera->nombre ?>" readonly>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                <div class="col-md-5 col-lg-4">
                    <div class="input-group input-group-sm">
                        ...
                        <input type="text" id="fechacreacion" name="fechacreacion" class="form-control form-control-sm" value="<?= $objEntidadFinanciera->fechaCreacion->format("m/d/Y H:i") ?>" readonly>
                    </div>
                </div>
                <div class="col-md-5 col-lg-3">
                    <div class="input-group input-group-sm">
                        <span class="input-group-text width-160px">User who created</span>
                        <input type="text" id="usuariocreo" name="usuariocreo" class="form-control form-control-sm" value="<?= $objEntidadFinanciera->usuarioCreo ?>" readonly>
                    </div>
                </div>
            </div>
            <div class="row mt-2">
                <div class="col-md-5 col-lg-4">
                    <div class="input-group input-group-sm">
                        ...
                        <input type="text" id="fechamodificacion" name="fechamodificacion" class="form-control form-control-sm" value="<?= $objEntidadFinanciera->fechaModificacion->format("m/d/Y H:i") ?>" readonly>
                    </div>
                </div>
                <div class="col-md-5 col-lg-3">
                    <div class="input-group input-group-sm">
                        <span class="input-group-text width-160px">User who modified</span>
                        <input type="text" id="usuariomodifico" name="usuariomodifico" class="form-control form-control-sm" value="<?= $objEntidadFinanciera->usuarioModifica ?>" readonly>
                    </div>
                </div>
            </div>
        </div>

        <input type="hidden" id="efid" name="efid" value="<?= $objEntidadFinanciera->financieraId ?>">
